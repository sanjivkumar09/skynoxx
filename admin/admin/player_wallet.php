...
$player_user_id = isset($_GET['player_id']) ? (int)$_GET['player_id'] : (int)$_GET['user_id'];

// Fetch player basic details
$u = null;
if ($st = $conn->prepare("SELECT id, name, email, joined_at FROM users WHERE id = ? AND role = 'player' LIMIT 1")) {
    $st->bind_param('i', $player_user_id);
    $st->execute();
    $res = $st->get_result();
    $u = $res->fetch_assoc();
    $st->close();
        ...
        try {
            if ($type === 'credit') {
                ...
                if ($upd = $conn->prepare("UPDATE users SET wallet_balance = wallet_balance + ? WHERE id = ?")) {
                    $upd->bind_param('di', $amount, $player_user_id);
                    $upd->execute();
                    $upd->close();
                }
                // Log transaction
                if ($log = $conn->prepare("INSERT INTO wallet_transactions (user_id, type, amount, status, description, created_at) VALUES (?, 'credit', ?, 'completed', ?, NOW())")) {
                    $log->bind_param('ids', $player_user_id, $amount, $reason);
                    $log->execute();
                    $log->close();
                }
                $adjustment_message = 'Successfully added ₹'.number_format($amount, 2).' to player wallet.';
                $adjustment_type = 'success';
            } else {
                // Deduct from wallet
                if ($upd = $conn->prepare("UPDATE users SET wallet_balance = wallet_balance - ? WHERE id = ? AND wallet_balance >= ?")) {
                    $upd->bind_param('did', $amount, $player_user_id, $amount);
                    $upd->execute();
                    ...
                    if ($affected === 1) {
                        ...
                        $adjustment_message = 'Successfully deducted ₹'.number_format($amount, 2).' from player wallet.';
                        $adjustment_type = 'success';
                    } else {
...
if ($wb = $conn->prepare("SELECT COALESCE(wallet_balance,0) AS bal FROM users WHERE id = ? LIMIT 1")) {
    $wb->bind_param('i', $player_user_id);
    $wb->execute();
    $rwb = $wb->get_result();
...
if ($tx = $conn->prepare("SELECT wt.id, wt.type, wt.amount, wt.status, wt.description, wt.tournament_id, wt.created_at,
                         ...
                         LIMIT 100")) {
    $tx->bind_param('i', $player_user_id);
    $tx->execute();
    $resTx = $tx->get_result();
...
// Name initials
$name_parts = explode(' ', $u['name'] ?? 'Player');
...
<head>
    ...
    <title>Player Wallet - Admin</title>
    ...
    <style>
        ...
        .content { max-width: 1400px; margin: 0 auto; padding: 2rem 1rem; }
        .card-dark { background: rgba(31,45,61,0.6); border: 1px solid var(--border); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; }
        ...
        .form-control { background: rgba(15,25,35,0.8); border: 1px solid var(--border); color: var(--text); }
        .form-control:focus { background: rgba(15,25,35,0.95); border-color: var(--primary); color: var(--text); }
    </style>
</head>
<body>
    <header class="admin-header">
        <div class="container d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="view_player_profile.php?user_id=<?php echo (int)$player_user_id; ?>" class="btn btn-outline-light btn-sm">
    ...
    <div class="content">
        <?php if ($adjustment_message): ?>
            <div class="alert alert-<?php echo $adjustment_type; ?> alert-dismissible fade show" role="alert">
            ...
            <div class="col-md-6">
                <div class="card-dark">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <div class="text-muted">Player</div>
        ...
        <div class="card-dark">
            <h5 class="mb-3"><i class="fas fa-edit me-2"></i>Adjust Wallet Balance (Admin Only)</h5>
            <form method="POST" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="reason" class="form-control" placeholder="Manual adjustment by admin" required>
                </div>
        ...
        <div class="card-dark">
                            ...
                            <?php foreach ($transactions as $txn): $t = strtolower((string)$txn['type']); $isCredit = in_array($t, ['credit','deposit','prize','prize_win']); ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php elseif (in_array($t, ['debit','withdrawal','deduct','entry_fee'])): ?>
            ...
            <?php else: ?>
                <div class="text-center text-muted py-4">
                    ...
                    <div>No transactions found for this player.</div>
                </div>
            <?php endif; ?>
        </div>
    </div>
